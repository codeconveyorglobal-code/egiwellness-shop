...
// Real product photo with a branded soft-pastel fallback. If the image fails to
// load (or none exists), the gradient tile shows instead — never looks broken.
export default function ProductMedia({ product, className = '', rounded = 'rounded-2xl' }) {
  ...
  return (
    ...
    >
      ...
      <div aria-hidden="true" className={`absolute inset-0 ${ok ? 'opacity-0' : 'opacity-100'} transition-opacity`}>
        ...
        <div
          ...
          style={{
            backgroundImage: 'radial-gradient(circle, rgba(20,32,27,0.25) 1px, transparent 1px)',
